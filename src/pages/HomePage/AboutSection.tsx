          ...
          >
            <div className="relative">
              <div className="relative aspect-[4/5] rounded-3xl overflow-hidden bg-gradient-to-br from-[#1a0a0d] via-[#0f0507] to-[#1a0a0d] border border-primary/20 group">
                ...
                {/* Blobs — scaled down and repositioned for mobile */}
                <motion.div
                  animate={{ y: [0, -12, 0], rotate: [0, 3, 0] }}
                  transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }}
                  className="absolute top-4 sm:top-8 -right-6 sm:-right-10 w-36 h-36 sm:w-48 sm:h-48 md:w-64 md:h-64 rounded-full bg-gradient-to-br from-primary/80 via-primary/60 to-primary/30 blur-[2px]"
                />
                <motion.div
                  animate={{ y: [0, 10, 0], x: [0, -6, 0] }}
                  transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
                  className="absolute top-16 sm:top-24 left-4 sm:left-6 w-20 h-20 sm:w-24 sm:h-24 md:w-32 md:h-32 rounded-full bg-gradient-to-br from-[#ffb3ba]/60 to-[#ff8fa3]/30 blur-[1px]"
                />
                <motion.div
                  animate={{ y: [0, -8, 0], x: [0, 8, 0] }}
                  transition={{ duration: 9, repeat: Infinity, ease: 'easeInOut', delay: 2 }}
                  className="absolute bottom-12 sm:bottom-16 right-8 sm:right-12 w-16 h-16 sm:w-20 sm:h-20 md:w-28 md:h-28 rounded-full bg-gradient-to-br from-[#faf0e6]/50 to-[#f5e6d3]/20"
                />

                {/* Memphis Shapes — scaled for mobile */}
                <svg className="absolute top-12 sm:top-16 left-2 sm:left-4 w-16 sm:w-20 h-8 sm:h-10 text-[#ffb3ba]/70" viewBox="0 0 80 40" fill="none">
                  <path d="M0 20 L10 5 L20 35 L30 5 L40 35 L50 5 L60 35 L70 5 L80 20" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
                </svg>
                <svg className="absolute bottom-24 sm:bottom-28 left-6 sm:left-8 w-24 sm:w-32 h-10 sm:h-12 text-primary/60" viewBox="0 0 128 48" fill="none">
                  <path d="M0 24 Q16 0 32 24 T64 24 T96 24 T128 24" stroke="currentColor" strokeWidth="3" strokeLinecap="round" />
                </svg>

                {/* Rings */}
                <div className="absolute top-8 sm:top-10 right-12 sm:right-16 w-12 sm:w-14 h-12 sm:h-14 rounded-full border-4 border-[#ffb3ba]/50" />
                <div className="absolute bottom-16 sm:bottom-20 right-6 sm:right-8 w-8 sm:w-10 h-8 sm:h-10 rounded-full border border-primary/50" style={{ borderWidth: '3px' }} />

                {/* Star */}
                <svg className="absolute bottom-32 sm:bottom-40 left-8 sm:left-10 w-6 sm:w-8 h-6 sm:h-8 text-[#faf0e6]" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 2l2.4 7.4h7.6l-6.2 4.5 2.4 7.4L12 16.8 5.8 21.3l2.4-7.4L2 9.4h7.6L12 2z" />
                </svg>
                <svg className="absolute top-1/2 right-10 sm:right-12 w-4 sm:w-5 h-4 sm:h-5 text-primary" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 2l2.4 7.4h7.6l-6.2 4.5 2.4 7.4L12 16.8 5.8 21.3l2.4-7.4L2 9.4h7.6L12 2z" />
                </svg>

                {/* Triangle */}
                <svg className="absolute top-1/3 right-4 sm:right-6 w-5 sm:w-6 h-5 sm:h-6 text-[#ffb3ba]/80" viewBox="0 0 24 24" fill="currentColor">
                ...
                <motion.div
                  ...
                  className="absolute bottom-6 sm:bottom-8 left-4 sm:left-6 right-4 sm:right-6"
                >
                  <div className="relative bg-background/70 backdrop-blur-md rounded-2xl p-4 sm:p-6 md:p-7 border border-primary/30 shadow-xl shadow-primary/10">
                    <div className="absolute -top-2 -right-2 w-5 sm:w-6 h-5 sm:h-6 rounded-full bg-primary flex items-center justify-center text-primary-foreground text-[10px] sm:text-xs font-bold">
                      ♥
                    </div>
                    ...
                    <h3 className="text-lg sm:text-xl md:text-2xl font-black italic">
                ...
                >
                  <div className="inline-flex items-center gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 bg-[#ffb3ba]/20 backdrop-blur-sm rounded-full border border-[#ffb3ba]/40 text-[#ffb3ba] text-[9px] sm:text-[10px] font-bold tracking-wider uppercase">
                    <span className="w-1 sm:w-1.5 h-1 sm:h-1.5 rounded-full bg-[#ffb3ba] animate-pulse" />
                    Francisco Entertainment
                  </div>
                </motion.div>
              </div>
              ...
              {/* Outer Decorations */}
              <div className="absolute -top-2 sm:-top-3 -right-2 sm:-right-3 w-12 sm:w-16 h-12 sm:h-16 rounded-full bg-primary/20 blur-xl" />
              <div className="absolute -bottom-3 sm:-bottom-4 -left-3 sm:-left-4 w-16 sm:w-20 h-16 sm:h-20 rounded-full bg-[#ffb3ba]/15 blur-xl" />
